...
class Net(nn.Module):
    def __init__(self, D=3, K=4):
        ...
        layers = []
        # Layer 1
        layers.append(nn.Linear(D, 128))  # Input to first hidden layer
        layers.append(nn.GELU())          # GELU activation
        layers.append(nn.Dropout(0.2))    # Dropout with rate 0.2

        # Layer 2
        layers.append(nn.Linear(128, 8))  # Second hidden layer
        layers.append(nn.Tanh())          # Tanh activation
        layers.append(nn.Dropout(0.1))    # Dropout with rate 0.1

        # Layer 3
        layers.append(nn.Linear(8, 16))   # Third hidden layer
        layers.append(nn.GELU())          # GELU activation

        # Layer 4
        layers.append(nn.Linear(16, 128)) # Fourth hidden layer
        layers.append(nn.Tanh())          # Tanh activation
        layers.append(nn.Dropout(0.1))    # Dropout with rate 0.1

        # Output Layer
        layers.append(nn.Linear(128, K))  # Output layer
...
def load_and_process_val_pcvc_data(directory='VowelData/.', train_size=0.8, random_seed=42):
...
best_model.load_state_dict(torch.load('ec24713_BestResult_best_net.pth', weights_only=True)) # Renamed the file manually, to not lose my best model

...
def load_and_process_pcvc_data(directory='VowelData/.', train_size=0.8, random_seed=42):
    """
...
class Net(nn.Module):
    ...
    def __init__(self, genome, D=3, K=4):
        ...
        input_features = D
        for gene in genome:
            layers.append(nn.Linear(input_features, gene['neurons']))
            if gene['activations'] == 'relu':
                layers.append(nn.ReLU())
            elif gene['activations'] == 'sigmoid':
                layers.append(nn.Sigmoid())
            elif gene['activations'] == 'tanh':
                layers.append(nn.Tanh())
            elif gene['activations'] == 'elu':
                layers.append(nn.ELU())
            elif gene['activations'] == 'leaky_relu':
                layers.append(nn.LeakyReLU())
            elif gene['activations'] == 'gelu':
                layers.append(nn.GELU())
            if gene['dropout'] is not None:
...
def generate_initial_population(size, blueprint):
    ...
    for _ in range(size):
        genome = []
        for _ in range(blueprint['max_n_layers']):
            layer = {
                'neurons': random.choice(blueprint['neurons']),
                'activations': random.choice(blueprint['activations']),
...
def selection(population, fitnesses, num_parents):
    ...
    population = list(zip(population, fitnesses))
    #sorted_population = sorted(population, key=lambda x: x[1], reverse=True)
    #top_performers = sorted_population[:num_parents]
    top_performers = heapq.nlargest(num_parents, population, key=lambda x: x[1])
    selected = [genome for genome, _ in top_performers]
...
def crossover(parent1, parent2):
    ...
    crossover_point = random.randint(0, min(len(parent1), len(parent2))-1) # We randomly select a crossover point
    child = parent1[:crossover_point] + parent2[crossover_point:] # We combine the parents' genes
...
def mutate(genome):
    ...
    """  
    layer_idx = random.randint(0, len(genome)-1) # We randomly select a layer to mutate
    mutation_type = random.choice(['neurons', 'activations', 'dropout']) # We randomly select a gene to mutate
    # We randomly select a new value for the gene, based on the mutation type 
    if mutation_type == 'neurons':
        genome[layer_idx]['neurons'] = random.choice([4, 8, 16, 32, 64, 128])
    elif mutation_type == 'activations':
        genome[layer_idx]['activations'] = random.choice(['relu', 'sigmoid', 'tanh', 'elu', 'leaky_relu', 'gelu'])
    elif mutation_type == 'dropout':
        genome[layer_idx]['dropout'] = random.choice([None, 0.1, 0.2, 0.35])
    return genome
...
blueprint = {
    'max_n_layers': 4,           # Define the (maximum) number of layers in each neural network
    'neurons': [4, 8, 16, 32, 64, 128],              # Possible neuron counts per layer
    'activations': ['relu', 'tunh', 'elu', 'sigmoid'],     # Possible activation functions
    'dropout': [None, 0.1, 0.2, 0.35]    #delete 4s        # Possible dropout rates, including 'None' for no dropout
}
...
for epoch in range(EPOCHS):
    ...
    print(f'Epoch {epoch + 1}/{EPOCHS} complete. Average Training Loss: {average_epoch_loss:.4f}')
    print(f"Best model's architecture: {best_overall_architecture}\n")
...
print(f'\n\nEvaluation on validation set complete. Accuracy: {validation_accuracy:.4f} ({correct}/{total} correct)')
print(f"Best model's architecture: {best_overall_architecture}")
print(f"choose hyperparameters: population_size = {population_size}, num_generations = {num_generations}, num_parents = {num_parents}, BATCH_SIZE = {BATCH_SIZE}, LR = {LR}, EPOCHS = {EPOCHS}")
for each in best_overall_architecture:
    print(each)
# Save the trained model's weights for future use.
torch.save(best_model.state_dict(), 'ec24713_best_net.pth')
